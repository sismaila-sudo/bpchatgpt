// Service temporaire avec localStorage en attendant les tables Supabase
...
export class HistoricalDataServiceMock {
  ...
  async getProjectHistoricalData(projectId: string): Promise<HistoricalData[]> {
    const data = localStorage.getItem(`historical_data_${projectId}`)
  ...
  async saveHistoricalData(data: Omit<HistoricalData, 'id' | 'created_at' | 'updated_at'>): Promise<HistoricalData> {
    ...
    const existing = await this.getProjectHistoricalData(data.project_id)

    // Remplacer ou ajouter
    const updated = existing.filter(
      item => !(item.year === data.year && item.data_type === data.data_type)
    )
    updated.push(historicalData)

    // Sauvegarder
    localStorage.setItem(`historical_data_${data.project_id}`, JSON.stringify(updated))
  ...
  async calculateAnalysis(projectId: string): Promise<FinancialAnalysis> {
    console.log('🧮 Calculating analysis for project:', projectId)
    ...
    console.log('📅 Data by year:', Object.keys(dataByYear))
    ...
    console.log('📊 Trends detected:', trends)
    ...
    localStorage.setItem(`financial_analysis_${projectId}`, JSON.stringify(analysisData))
  ...
  async getAnalysis(projectId: string): Promise<FinancialAnalysis | null> {
    const data = localStorage.getItem(`financial_analysis_${projectId}`)
  ...
  private calculateKPIs(dataByYear: Record<number, Record<string, any>>) {
    ...
    years.forEach(year => {
      ...
      if (yearData.bilan && yearData.compte_resultat) {
        ...
        ratios.push({
          year,
          rentabilite: cr.chiffre_affaires > 0 ? (cr.resultat_net / cr.chiffre_affaires * 100) : 0,
          liquidite: bilan.dettes_court_terme > 0 ? (bilan.actif_circulant / bilan.dettes_court_terme) : 1,
          autonomie: bilan.total_passif > 0 ? (bilan.capitaux_propres / bilan.total_passif * 100) : 0
        })
      }
    })
  ...
  private detectTrends(kpis: any) {
    const detectTrend = (values: number[]) => {
      ...
      if (lastTwo[0] === 0) return lastTwo[1] > 0 ? 'up' : 'stable'
  ...
  private generateAICommentary(kpis: any, trends: any): string {
    const caValues = kpis.ca_evolution.filter((v: number) => v > 0)
    const resultValues = kpis.resultat_evolution.filter((v: number) => v !== 0)

    const caGrowth = caValues.length > 1
      ? ((caValues[caValues.length - 1] - caValues[0]) / caValues[0] * 100)
      : 0
  ...
  private generateRecommendations(trends: any, healthScore: number): string[] {
    ...
    if (healthScore > 70) {
      ...
      recommendations.push("Envisager l'acquisition d'équipements pour augmenter la capacité")
...
export const historicalDataService = new HistoricalDataServiceMock()